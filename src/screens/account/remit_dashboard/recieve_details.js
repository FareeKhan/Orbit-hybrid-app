...
const RecieveDetails = ({route, navigation, homeActions,authToken }) => {
  ...
  return (
    <View style={styles.mainBody}>
      ...
      <NormalHeader title={"Remit Recieve Details"} />
        <ScrollView
          ...
          {EachTab("Sender Name", data.sender_fname + " " + (data.sender_lname == null ? "" : data.sender_lname ))}
          {EachTab("Sender Phone", data.sender_phone)}
          {EachTab("Sender Email", data.sender_email)}
          {EachTab("Payment Status", data.payment_status)}
          {EachTab("Payment Date", moment(data.payment_date).format('YYYY-MM-DD'))}
          {EachTab("Remit Amount", "$"+data.payment_amount)}
          {EachTab("Bank", data.bank_name)}
          {EachTab("Account Number", data.account_number)}
          {EachTab("Process Status", trxStatus(data.txn_status))}
          {EachTab("Process TXN ID", data.payment_txnid)}
          {EachTab("Process Date", moment(data.created_at).format('YYYY-MM-DD'))}
...
export default connect(mapStateToProps, mapDispatchToProps)(RecieveDetails);